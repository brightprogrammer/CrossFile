/* This file defines standard ELF types, structures, and macros.
   Copyright
 * (C) 1995-2018 Free Software Foundation, Inc.
...
/* Fields in the e_ident array.  The EI_* macros are indices into the
   array.
 * The macros under each EI_* macro are the values the byte
   may have.  */
...
/* If it is necessary to assign new unofficial EM_* values, please
   pick large
 * random numbers (0x8523, 0xa7f2, etc.) to minimize the
...
/* Symbol table indices are found in the hash buckets and chain table
   of a
 * symbol hash table section.  This special index value indicates
   the end of
 * a chain, meaning no further symbols are found in that bucket.  */
...
/* Special value for e_phnum.  This indicates that the real number of
   program
 * headers is too large to fit into e_phnum.  Instead the real
   value is in
 * the field sh_info of section 0.  */
 ...
 * to the ELF object after it has been
   built these entries will need to be
 * adjusted.  */
...
/* This vector is normally only used by the program interpreter.  The
   usual
 * definition in an ABI supplement uses the name auxv_t.  The
   vector is not
 * usually defined in a standard <elf.h> file, but it
   can't hurt.  We rename
 * it to avoid conflicts.  The sizes of these
   types are an arrangement
 * between the exec server and the program
   interpreter, so we don't fully
 * specify them here.  */
...
typedef struct {
 ...
 though,
     * since it does not work when using 32-bit definitions
       on 64-bit
...
/* Shapes of the caches, with more room to describe them.
   *GEOMETRY are
 * comprised of cache line size in bytes in the bottom 16 bits
   and the cache
 * associativity in the next 16 bits.  */
 ...
 * variable-length entries, one byte followed by a
   '\0'-terminated hwcap name
 * string.  The byte gives the bit
   number to test if enabled, (1U << bit) &
 * bitmask.  */
...
/* The x86 instruction sets indicated by the corresponding bits are
   used in
 * program.  Their support in the hardware is optional.  */
#define GNU_PROPERTY_X86_ISA_1_USED 0xc0000000
/* The x86 instruction sets indicated by the corresponding bits are
   used in
 * program and they must be supported by the hardware.   */
...
typedef struct {
 ...
 variable part of descriptor. */
    ...
    Elf32_Section
               section; /* Section header index of section affected,
 0 for
                            global options.  */
...
/* The base of the PLT in an executable using the new non-PIC ABI if that
   PLT
 * is writable.  For a non-writable PLT, this is omitted or has a zero
   value.
 */
#define DT_MIPS_RWPLT 0x70000034
/* An alternative description of the classic MIPS RLD_MAP that is usable
   in a
 * PIE as it stores a relative offset from the address of the tag
   rather than
 * an absolute address.  */
...
#define R_PPC64_DTPREL16_HIGHESTA                                                                  \
    106                                /* half16    (sym+add)@dtprel@highesta   \
                                       */
...
/* e_flags bits specifying ABI.
   1 for original function descriptor using
 * ABI,
   2 for revised ABI without function descriptors,
   0 for unspecified
 * or not using any features affected by the differences.  */
